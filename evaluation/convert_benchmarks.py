...
Currently supports one subcommand:
  ...
  - `interns1_504` — converts `interns1_math_18domains_504.json` (the
    ...
    raw_metadata}` JSONL schema consumed by `runtime.run` and
    `evaluation.evaluate`.
...
# MathSolution `answer_type` enum (see shared/vendor/parser.py for the
...
# domain ids used in shared/vendor/parser.py:DOMAIN_VALUES. Unknown

...
# runs first. The solver imports `from llm_math.prompts import get_prompt`,
...
import llm_math  # noqa: F401


def _reload_prompt_modules() -> None:
    """Re-import every non-`_`-prefixed module in `llm_math.prompts`.

    Mirrors the discovery logic in `llm_math.prompts._discover_prompts()` so
    that `reset_for_tests()` actually clears the cache rather than relying
    on a hardcoded list of names.
    """
    import llm_math.prompts as prompts_pkg
